...
module pcm_oct_m
  ...
  !GGIL LOG: 31/05/2017 - Linking TDPlas
  use eom_pcm_tdplas_m!, only: pcm_charges_propagation, debye_param_t, drude_param_t 
...
contains
  ...
  subroutine pcm_init(pcm, geo, grid, qtot, val_charge)
    ...
    !% For the moment, the choices of equation of motion ensue from either Debye (default) or Drude-Lorentz model.
    ...
    if(pcm%which_eps/='deb'.and.pcm%which_eps/='drl') then
      call messages_write('Sorry, only Debye or Drude-Lorentz dielectric model are available.')
    ...
    !GGIL LOG: 31/05/2017 - Re-parse TDTimeStep to propagate polarization charges
    ...
    !GGIL: - added
    !%Variable PCMPlasmonFrequency
    !%Type float
    !%Default <math>\sqrt{1/(\varepsilon_0-1)}</math>
    !%Section Hamiltonian::PCM
    !%Description
    !% Plasmon frequency of the solvent within Drude-Lorentz model (<math>\omega_0</math>).
    ...
    !%End
    call parse_variable('PCMPlasmonFrequency', sqrt(M_ONE/(pcm%epsilon_0-M_ONE)), pcm%drl%w0)
    call messages_print_var_value(stdout, "PCMPlasmonFrequency", pcm%drl%w0)
...
!     end do
  ...
  subroutine pcm_calc_pot_rs(pcm, mesh, geo, v_h, time_present)
    ...
    if (calc == PCM_ELECTRONS) then
      ...
      if( td_calc_mode .and. pcm%epsilon_infty /= pcm%epsilon_0 .and. (pcm%noneq.or.pcm%eom) ) then
        if( pcm%eom ) then
          !GGIL LOG: 01/06/2017 - Linking to TDPlas
        ...
        end if !GGIL LOG: 01/06/2017 - Linking to TDPlas or proceeding with inertial/dynamical charge splitting
